...
publishing over newer local truth in one JavaScript realm. Web publication also
takes a bounded exact-scope Web Lock shared by same-origin tabs/windows in the
same storage partition, then
rereads one count-complete owner snapshot with the captured bearer and abort
