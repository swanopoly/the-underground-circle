...
const TOOL_DEFINITIONS: OpenSwanToolDefinition[] = [
  ...
  },
  {
    ...
    surfaces: ['main_chat', 'room_chat', 'task_run'],
    description: 'Temporarily fail-closed until the runtime binds and rechecks an exact browser process/context/page/opaque-URL plus exact field fingerprint at handler entry. Raw secrets are never returned to the model. Ask the user to enter credentials manually; OTP, MFA, CAPTCHA, and bot checks always require the human.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    surfaces: ['main_chat', 'room_chat', 'task_run'],
    description:
      'One-call part design: state a DUTY, get a sized part — dimensions, Blender model, mass, realised safety factor, bore fit. Sizes the member, ROUNDS up to stock, re-checks. Types: bracket (load, arm), shaft (torque), beam (load, span), gearbox (power_kW, inputSpeed_rpm, ratio), isolator (mass_kg, speed_rpm, isolationPercent), pressure_cover (pressure_MPa, boreDiameter_mm), conveyor_drive (power_kW, inputSpeed_rpm, ratio), brake (torque_Nm, speed_rpm). Use for "design a <part> to carry/transmit/isolate/stop X"; compile the script with desktop.cad_compile, then engineering.inspect_mesh to verify.',
    inputSchema: {
      type: 'object',
      properties: {
        type: { type: 'string', description: 'bracket | shaft | beam | gearbox | isolator | pressure_cover | conveyor_drive | brake.' },
        ...
        safetyFactor: { type: 'number', description: 'Target safety factor (default 2; pressure_cover defaults 3).' },
        ...
        power_kW: { type: 'number', description: 'Transmitted power in kW (gearbox/conveyor_drive).' },
        inputSpeed_rpm: { type: 'number', description: 'Input shaft speed in rpm (gearbox/conveyor_drive).' },
        ratio: { type: 'number', description: 'Speed reduction ratio (gearbox/conveyor_drive).' },
        targetLifeHours: { type: 'number', description: 'Bearing L10 life target in hours (gearbox/conveyor_drive, default 20000).' },
        mass_kg: { type: 'number', description: 'Supported machine mass in kg (isolator).' },
        speed_rpm: { type: 'number', description: 'Machine/shaft speed in rpm (isolator disturbance, brake).' },
        isolationPercent: { type: 'number', description: 'Vibration isolation target in % (isolator, e.g. 90).' },
        springCount: { type: 'number', description: 'Number of mount springs (isolator, default 4).' },
        pressure_MPa: { type: 'number', description: 'Internal pressure in MPa (pressure_cover).' },
        boreDiameter_mm: { type: 'number', description: 'Vessel bore diameter in mm (pressure_cover).' },
        boltCount: { type: 'number', description: 'Flange bolt count override (pressure_cover; default sized, multiple of 4).' },
        torque_Nm: { type: 'number', description: 'Required braking torque in N·m (brake).' },
        dutyCycle: { type: 'number', description: 'Braking duty fraction for continuous heat (brake, default 0.1).' },
        maxTempRise_C: { type: 'number', description: 'Allowed steady temperature rise in °C (brake, default 60).' },
...
export function getOpenSwanToolParallelPolicy(
  ...
  // Temporal waits and semantic scrolling are sequence primitives, not
  // order-free reads: same-round calls the model emits after them depend on
  // the wait/viewport move having completed. Their base policies stay
  // auto-approved, but parallel dispatch treats them as singleton barriers.
