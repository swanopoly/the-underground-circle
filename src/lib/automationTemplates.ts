...
If the PR action is not "opened" or "synchronize", respond with SKIP.`,
  ...
  {
    id: 'nudge-inactive-members',
    name: 'Nudge Inactive Members',
    icon: '👀',
    description: 'Nudge members who haven\'t pushed code in 3+ days',
    category: 'github',
    trigger_type: 'schedule',
    ...
    model: 'claude-haiku',
    output_target: 'chat',
    prompt: "The following members of {{circle_name}} haven't pushed code in 3+ days:\n\n{{inactive_members}}\n\nWrite a brief, direct nudge for the team chat. Mention each inactive member by name. Be motivating but honest — shipping is what matters. Keep it under 100 words. End with a call to action.",
    include_context: { members: true },
    spirit: 'coach',
    suggested: true,
    suggestedIconBg: '#f59e0b',
  },
  {
    id: 'deploy-failure-alert',
    name: 'Deploy Failure Alert',
    icon: '🚨',
    description: 'Alert the team when CI/CD pipelines fail',
    category: 'github',
    trigger_type: 'schedule',
    cron_expression: '*/15 * * * *',
    agent: 'BlackSwan',
    model: 'claude-haiku',
    output_target: 'chat',
    prompt: "The following CI/CD failures were detected for {{circle_name}}:\n\n{{deploy_failures}}\n\nFor each failure, write a brief alert: what workflow failed, on which repo, the likely cause, and a direct link. Be concise and actionable. If the same workflow failed multiple times, group them.",
    include_context: { members: true },
    spirit: 'devops',
    suggested: true,
    suggestedIconBg: '#ef4444',
  },

  // ─── Mission Automations ──────────────────────────────────────────────────
  {
    id: 'mission-daily-digest',
    name: 'Daily Mission Digest',
    icon: '🎯',
    description: 'BlackSwan posts a daily summary of all active missions — progress, blockers, and who needs to ship',
    category: 'accountability',
    trigger_type: 'schedule',
    cron_expression: 'daily',
    agent: 'BlackSwan',
    model: 'claude-haiku',
    output_target: 'chat',
    prompt: "You are BlackSwan, the AI accountability agent for {{circle_name}}. Generate a concise daily mission digest.\n\nFor each active mission in the circle:\n1. Show the mission title, progress percentage, and deadline\n2. List remaining tasks (max 3 per mission)\n3. Flag any overdue missions with urgency\n4. Call out members who haven't completed any tasks recently\n\nEnd with one motivational nudge. Keep it under 250 words. Be direct, not fluffy.",
    include_context: { members: true, tasks: true, check_ins: true, streaks: true, analytics: false, rooms: false, goals: true },
    spirit: 'pm',
    suggested: true,
    suggestedIconBg: '#6366f1',
  },
  {
    id: 'mission-overdue-nudge',
    name: 'Overdue Mission Nudge',
    icon: '!!',
    description: 'Alert the circle when missions are past their deadline with specific action items',
    category: 'accountability',
    trigger_type: 'schedule',
    cron_expression: 'daily',
    agent: 'BlackSwan',
    model: 'claude-haiku',
    output_target: 'chat',
    prompt: "Check all active missions for {{circle_name}}. For any mission that is past its deadline:\n1. Name the mission and how many days overdue\n2. Show incomplete tasks and who owns them\n3. Suggest whether to extend the deadline, archive it, or push to complete\n\nIf no missions are overdue, say nothing (output empty string). Be direct and constructive, not passive-aggressive.",
    include_context: { members: true, tasks: true },
    spirit: 'pm',
  },
  {
    id: 'mission-weekly-retro',
    name: 'Weekly Mission Retrospective',
    icon: '[]',
    description: 'End-of-week summary of missions completed, in-progress, and proof-of-work highlights',
    category: 'reporting',
    trigger_type: 'schedule',
    cron_expression: 'weekly',
    agent: 'BlackSwan',
    model: 'claude-haiku',
    output_target: 'chat',
    prompt: "Generate a weekly retrospective for {{circle_name}}.\n\nCover:\n1. Missions completed this week (celebrate them!)\n2. Active missions and their progress\n3. Top contributors — who shipped the most tasks/commits\n4. Proof-of-work highlights — notable commits, PRs, or agent completions\n5. One recommendation for next week\n\nKeep it under 300 words. Make it feel like a team standup, not a corporate report.",
    include_context: { members: true, tasks: true, check_ins: true, streaks: true, analytics: true, goals: true },
    spirit: 'pm',
    suggested: true,
    suggestedIconBg: '#22c55e',
  },
  {
    id: 'mission-completion-celebration',
    name: 'Mission Completion Celebration',
    icon: 'OK',
    description: 'BlackSwan celebrates when a mission is marked complete — shout out contributors',
    category: 'engagement',
    trigger_type: 'event',
    event_config: { table: 'circle_missions', event: 'UPDATE' },
    agent: 'BlackSwan',
    model: 'claude-haiku',
    output_target: 'chat',
    prompt: "A mission was just completed in {{circle_name}}! Write a brief celebration message (3-4 sentences max). Mention the mission name, how many tasks were done, and give a shout-out to the team. Keep the energy high but authentic — no corporate cringe.",
    include_context: { members: true, tasks: true },
    spirit: 'hype',
  },
];
