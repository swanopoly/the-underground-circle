...
// above augments a *single* repeated call's feedback; the primitives below let
// a loop take a HARD terminal decision when it sees ~3 identical failing calls
// in a row across recent rounds.
//
// P61: the implementations live in `toolLoopStuckCore.ts` (ZERO imports) so
// Deno edge functions can consume them under strict module resolution; this
// re-export keeps every existing client import working unchanged. The
// canonicalization in `hashToolInput` intentionally matches
// `toolCallSignature` above so the two agree on "the same input".
export {
