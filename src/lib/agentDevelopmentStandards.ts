...
const AGENT_WORKTREE_OWNER_RULES: AgentWorktreeOwnerRule[] = [
  ...
  },
  {
    ...
    matchers: [
      /^src\/lib\/chat(?:Actions|AgentIdentity|AgentService|ApprovalGate|AutomationPlanner|CommandRegistry|Recording|SessionArchive|SessionArchivePrompt|SessionArchiveRecovery|AgentTargets|DesignTaskCard|DesktopAttachmentRouting|RecoveryActionIntent|FailureRecovery)\.ts$/,
      ...
      /^scripts\/(?:automation-cadence-format|chat-approval-single-use|chat-planner|chat-recording|persisted-chat-metadata|chat-agent-targets|chat-automation|chat-conversational|chat-design-task-card|chat-desktop-attachment-routing|chat-failure-recovery|chat-terminal|chat-transport-handlers|predictive-chat-commands|simple-chat-task-guardrails|user-task-pipelines)-smoketest\.ts$/,
      /^scripts\/(?:chat-lane-outcome|chat-plan-tool-manifest)-smoketest\.ts$/,
  ...
  },
  {
    ...
    matchers: [
      ...
      /^scripts\/photoshop-extendscript-adapters-smoketest\.ts$/,
    ],
    canonicalDocs: [
      'docs/AGENTS_ROADMAP.md',
      'docs/UC_APP_STACK_REFERENCE.md',
  ...
  },
  {
    ...
    matchers: [
      ...
      /^src\/lib\/computer(?:App|Task|Use|File|Capability|Grant|Desktop|Sequence|Foreground)/,
      ...
      /^src\/lib\/browser(?:AI|Bridge|Task|Primitives)/,
      ...
      /^scripts\/(?:a11y-tree|app-learned-facts|app-task-resolver|chat-computer|computer-app|computer-task|computer-sequence|computer-capability-expansion|computer-grant-gate|computer-pipeline-e2e|computer-use-queue|desktop-|direct-image-conversion|direct-local-file|browser-|photoshop-save-dialog|local-desktop|scriptable-mac-apps)/,
      /^scripts\/(?:chat-file-permission-demand|computer-foreground-ownership)-smoketest\.ts$/,
  ...
  },
  {
    ...
    matchers: [
      ...
      /^src\/lib\/swanbot\.ts$/,
      /^src\/lib\/swanbot(?:ClientToolDispatcher|OpenSwanReadiness|TurnDedupe|V2BatchPolicy|V2BatchRuntime|V2DispatcherParity|V2StopReason)\.ts$/,
  ...
  },
  {
    ...
    matchers: [
      /^docs\/(?:CHAT_APP_AUTOMATION_DIRECTION|CHAT_AUTOMATION_AUDIT_PLAN|CHAT_SWANBOT_OPENSWAN|CURSOR_AGENT_FEATURE_PARITY_PLAN|DESKTOP_MANIPULATION_PLAN|DIRECT_DESKTOP_TASK_EXPANSION_RESEARCH|EXECUTION_LADDER_RESEARCH|LEARNING_LOOP_RESEARCH|SWANBOT_OPENSWAN_|SWANBOT_PIPELINE_RESEARCH|SWANBOT_V2_MIGRATION_PLAN)/,
