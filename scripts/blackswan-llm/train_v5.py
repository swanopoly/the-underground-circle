#!/usr/bin/env python3
"""
BlackSwan LLM v5 — "The Big Boy"
Fine-tune Qwen3.5-27B with QLoRA for the full BlackSwan model.

Requirements:
  - 56GB+ VRAM/unified memory (Apple Silicon Mac with 64GB+ or multi-GPU Linux)
  - Unsloth with Qwen3.5 support
  - Same dataset as v4 (43K+ examples)

Architecture:
  - Base: Qwen3.5-27B (dense, Apache 2.0)
  - QLoRA: rank 64, alpha 128, 4-bit quantization
  - Training: 1 epoch, batch 1, grad_accum 16 (effective batch 16)
  - Export: GGUF Q4_K_M for Ollama deployment

Usage:
  python train_v5.py [--epochs 1] [--lr 5e-5] [--batch 1]
  python train_v5.py --base-model unsloth/Qwen3.5-9B-bnb-4bit  # smaller fallback
"""
...
TRAIN_FILE = DATA_DIR / "train_v4.jsonl"    # Reuses v4 dataset (43K examples)
EVAL_FILE = DATA_DIR / "eval_v4.jsonl"
OUTPUT_DIR = Path(__file__).parent / "models" / "v5"


def main():
    parser = argparse.ArgumentParser(description="Fine-tune BlackSwan LLM v5 (Qwen3.5-27B)")
    ...
    parser.add_argument("--base-model", type=str, default=BASE_MODEL,
                        help="Override base model (e.g. unsloth/Qwen3.5-9B-bnb-4bit)")
    args = parser.parse_args()

    is_mac = platform.system() == "Darwin"
    if is_mac:
        print("Detected Apple Silicon Mac — using MPS backend")
        print("  Requires 64GB+ unified memory for 27B model")
    ...
    print(f"\nLoading {base_model}...")
    print("  (This may take several minutes for a 27B model)")
    ...
    print(f"\nTraining config (v5 — BlackSwan Full):")
    ...
    print("\nStarting v5 training (BlackSwan Full — 27B)...")
    print("  This will take a while. Go touch grass.\n")
