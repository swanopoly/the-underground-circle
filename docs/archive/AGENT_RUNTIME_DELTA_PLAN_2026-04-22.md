...
- **✗ missing.** `AGENT_RUNTIME_INTEGRATION_PLAN.md` Phase 5 was always gated on "≥50 skills + ≥1K persisted runs." We're not there yet. Our regression-benchmark file (`openswanBenchmarks.ts`) is a test suite, not an optimizer.
...
- DSPy/GEPA full pipeline — blocked on #7 (trace export) + the ≥50 skills / ≥1K runs gate already in `AGENT_RUNTIME_INTEGRATION_PLAN.md` Phase 5.
