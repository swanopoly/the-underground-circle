...
M3 is complete. The v2 edge catalog is source-derived from `supabase/functions/swanbot-v2-ai/index.ts` and currently pins **79 tools**: **25 server-side** and **54 client-delegated** (coding-agent client tools — edit_file, run_shell, git.run, codebase.search, todo.write, coordination.file_status — added 2026-07-14). The historical sub-phases below record how parity was reached; new catalog growth must be re-pinned by `smoke:swanbot-openswan-readiness` and the dispatcher-parity smoke before it is treated as ready.
