-- ============================================================
-- Simplified Combined migration: only tables, columns, RLS
-- Run this in Supabase SQL Editor. If it succeeds, the issue is
-- in the commented-out functions or seed data.
-- ============================================================

-- ─── 1. Discord Integration (circles columns + discord_channels table) ───
...
-- ─── 2. Gamification (xp_events, user_xp, achievements, votes, etc.) ───
...
DROP POLICY IF EXISTS "xp_events_insert" ON xp_events FOR INSERT WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Members can create challenges"
  ...
  WITH CHECK (created_by = auth.uid() AND circle_id IN (SELECT circle_id FROM circle_members WHERE user_id = auth.uid()));
...
CREATE POLICY "Members can read challenge participants"
  ...
  USING (challenge_id IN (SELECT c.id FROM challenges c JOIN circle_members cm ON cm.circle_id = c.circle_id WHERE cm.user_id = auth.uid()));
...
-- ─── Commented out for debugging: get_user_circles RPC ───
...
-- ─── 5. Circle settings columns (from 20260217) ───
...
-- ============================================================
-- Done! All missing tables, columns, RPCs, and RLS policies.
-- ============================================================
