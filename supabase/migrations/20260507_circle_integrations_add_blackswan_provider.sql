-- Add `blackswan` as its own first-class circle_integrations provider
-- so the marketplace can carry a dedicated BlackSwan card with the
-- Inference Endpoint URL + auth token in one place. Members of a
-- circle inherit the connection — no per-user setup.
--
-- Run via Supabase SQL Editor (the migration system in this repo
-- doesn't auto-apply).
