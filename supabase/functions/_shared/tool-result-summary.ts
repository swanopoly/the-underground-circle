// tool-result-summary — DENO LOCKSTEP MIRROR of src/lib/toolResultSummaryCore.ts.
//
// The v2 edge tool-loop (swanbot-v2-ai via _shared/swanbot-continuation.ts) must
// summarize oversized client tool results EXACTLY the way the client core does,
// so the edge and client never diverge on what the model sees. Because the
// source core is PURE (zero runtime imports), this is a byte-for-byte-equivalent
// copy: same tunables, same newline-snap logic, same signal regex, same dedupe
// and caps → same output for the same input. KEEP THIS FILE IN STEP with
// src/lib/toolResultSummaryCore.ts; any change there must be mirrored here (and
// the lockstep is asserted by scripts/edge-tool-result-summary-smoketest.ts,
// which compares both implementations on identical inputs).
//
// ── original core doc (verbatim) ─────────────────────────────────────────────
// The PURE deterministic tool-result summarizer for large tool outputs (P6 of
// docs/CODING_AGENT_UPGRADE_PLAN.md). A single `npm test` or build tool_result
// can be 300KB; stringifying `{ok:true,data}` into the model context floods the
// window and buries the signal. This core keeps the HEAD and TAIL of the output
// verbatim (snapped to line boundaries when a newline is nearby, so lines aren't
// split mid-way), drops the middle, and surfaces "signal lines" from the omitted
// region — lines that look like errors/warnings/failures/timeouts — deduped and
// capped. A marker line states exactly what was kept and omitted so the model
// never mistakes the summary for the full output.
//
...
//
// PURITY: zero runtime imports, valid Deno TS (no node builtins) and tsx-loadable.
// This module never touches the filesystem or network.
